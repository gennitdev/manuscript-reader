...
const router = createRouter({
  ...
  scrollBehavior(to, from, savedPosition) {
    ...
    if (to.params.chapterId !== from.params.chapterId) return { top: 0 }
    return false
  },
})
...
router.afterEach((to) => {
  ...
  document.title = chapter ? `${chapter.title || 'Untitled'} · ${book?.title || 'Manuscript'}` : book?.title || 'Manuscript Reader'
